...
<form id="CCformVenta" action="{{ route('CCvent.CCstoremobile') }}" method="POST">
    @csrf
    ...
    <h1>Productos pedidos para confirmación</h1>
    @foreach ($ventacabecera as $item)
    <div class="card"
            ...
            <div class="controls">
                ...
                <input type="number" class="qty" name="arraycantidad[]" value="{{ $item->cantidad }}" min="0">
                <button type="button" class="btn-circle btn-plus">+</button>
            </div>

            <div class="price">
                Descuento Q.
                <input type="number" class="descuento" name="arrayDescuento[]" value="{{ $item->descuento }}" min="0">
            </div>

            <div class="price">
                Subtotal Q.
                <span class="subtotal">{{$item->subtotal}}</span>
                <input class="subtotalinput" type="hidden" name="Subtotal[]" value="{{ $item->subtotal }}">
            </div>

    </div>
    @endforeach
<h1>Productos no vendidos</h1>
    ...
    <div class="card-prod"
            ...
            <div class="controls">
                <button type="button" class="btn-circle btn-minus">-</button>
                <input type="number" class="qty" name="arraycantidad[]" value="0" min="0" max="{{ $item->stock }}">
                <button type="button" class="btn-circle btn-plus">+</button>
            </div>
    ...
    @endforeach
        ...
        <input type="hidden" name="comprobante_id" id="inputDocumento">

<button type="button" class="fab-enviar" data-bs-toggle="modal" data-bs-target="#modalDocumento">
    💾
</button>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-success" onclick="confirmarGuardar()">Confirmar</button>
            </div>
...
function confirmarGuardar() {
        ...
        let form = document.getElementById("CCformVenta");
    ...
    let documento = document.getElementById("documento_id").value;
    ...
    form.action = "{{ route('CCvent.CCstoremobile') }}";
...
let escaneando = false;
    ...
    btnPlus.addEventListener("click", () => {
        let stock = parseInt(card.querySelector("h6").innerText.replace(/\D/g, '')) || 0;
let actual = parseInt(qtyInput.value || 0);

if (actual < stock) {
    qtyInput.value = actual + 1;
} else {
    Swal.fire({
    icon: 'warning',
    title: 'Stock insuficiente',
    text: 'No puedes agregar más de lo disponible'
});
}

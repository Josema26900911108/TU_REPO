<div id="tabla_pago_content">

    <div class="card-body border-bottom">
        <div class="row g-3">
            <div class="col-md-3">
                <label class="form-label">Buscar general:</label>
                <input type="text" id="globalSearch" class="form-control" placeholder="Buscar...">
            </div>
        </div>
    </div>
<div class="table-responsive w-100 mt-3">
    <table id="datatablesSimple" class="table table-striped fs-6">
        <thead>
            <tr>
                <th>Serie</th>
                <th>SKU</th>
                <th>Almacén</th>
                <th>Lote</th>
                <th>Cantidad</th>
                <th>UM</th>
                <th>MAC1</th>
                <th>MAC2</th>
                <th>Estatus</th>
                <th>Centro</th>
                <th>Creado Por</th>
                <th>Fecha</th>
                
                @can('vertienda-producto')
                <th>Tienda</th>
                @endcan
                <th>Acciones</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($movimientos as $item)
            <tr>
                <td>{{ $item->serie ?? 'N/A' }}</td>
                <td>{{ $item->SKU }}</td>
                <td>{{ $item->almacen }}</td>
                <td>{{ $item->Lote ?? 'N/A' }}</td>
                <td>{{ number_format($item->cantidad, 2) }}</td>
                <td>{{ $item->unidadmedida }}</td>
                <td>{{ $item->MAC1 ?? '-' }}</td>
                <td>{{ $item->MAC2 ?? '-' }}</td>
                <td>
                    <span class="badge {{ $item->ESTATUS == 'ACTIVO' ? 'text-bg-success' : 'text-bg-secondary' }}">
                        {{ $item->ESTATUS }}
                    </span>
                </td>
                <td>{{ $item->CENTRO }}</td>
                <td>{{ $item->Creado_por }}</td>
                <td>{{ date('d-m-Y', strtotime($item->Creado_el)) }}</td>

                @can('vertienda-producto')
                <td>
                    {{ $item->fkTienda ?? 'Sin tienda asignada' }}
                </td>
                @endcan

                <td>
                    <div class="d-flex justify-content-around">
                        <div>
                            <button title="Opciones" class="btn btn-datatable btn-icon btn-transparent-dark me-2" data-bs-toggle="dropdown" aria-expanded="false">
                                <svg class="svg-inline--fa fa-ellipsis-vertical" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="ellipsis-vertical" role="img" xmlns="http://w3.org" viewBox="0 0 128 512">
                                    <path fill="currentColor" d="M56 472a56 56 0 1 1 0-112 56 56 0 1 1 0 112zm0-160a56 56 0 1 1 0-112 56 56 0 1 1 0 112zM0 96a56 56 0 1 1 112 0A56 56 0 1 1 0 96z"></path>
                                </svg>
                            </button>
                            <ul class="dropdown-menu text-bg-light" style="font-size: small;">
                                @can('ver-producto')
                                <li>
                                    <a class="dropdown-item" role="button" data-bs-toggle="modal" data-bs-target="#verModal-{{$item->id}}">Ver detalles</a>
                                </li>
                                @endcan
                            </ul>
                        </div>
                        <div>
                            <div class="vr"></div>
                        </div>
                        <div>
                            @can('eliminar-eta')
                            <button title="Eliminar" data-bs-toggle="modal" data-bs-target="#confirmModal-{{$item->id}}" class="btn btn-datatable btn-icon btn-transparent-dark">
                                <svg class="svg-inline--fa fa-trash-can" aria-hidden="true" focusable="false" data-prefix="far" data-icon="trash-can" role="img" xmlns="http://w3.org" viewBox="0 0 448 512">
                                    <path fill="currentColor" d="M170.5 51.6L151.5 80h145l-19-28.4c-1.5-2.2-4-3.6-6.7-3.6H177.1c-2.7 0-5.2 1.3-6.7 3.6zm147-26.6L354.2 80H368h48 8c13.3 0 24 10.7 24 24s-10.7 24-24 24h-8V432c0 44.2-35.8 80-80 80H112c-44.2 0-80-35.8-80-80V128H24c-13.3 0-24-10.7-24-24S10.7 80 24 80h8H80 93.8l36.7-55.1C140.9 9.4 158.4 0 177.1 0h93.7c18.7 0 36.2 9.4 46.6 24.9zM80 128V432c0 17.7 14.3 32 32 32H336c17.7 0 32-14.3 32-32V128H80zm80 64V400c0 8.8-7.2 16-16 16s-16-7.2-16-16V192c0-8.8 7.2-16 16-16s16 7.2 16 16zm80 0V400c0 8.8-7.2 16-16 16s-16-7.2-16-16V192c0-8.8 7.2-16 16-16s16 7.2 16 16zm80 0V400c0 8.8-7.2 16-16 16s-16-7.2-16-16V192c0-8.8 7.2-16 16-16s16 7.2 16 16z"></path>
                                </svg>
                            </button>
                            @endcan
                        </div>
                    </div>
                </td>
            </tr>

            <!-- Modal Ver Detalles -->
            <div class="modal fade" id="verModal-{{$item->id}}" tabindex="-1" aria-labelledby="verModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-scrollable">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="verModalLabel">Detalles del Movimiento de Material</h1>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="row g-2">
                                <div class="col-12"><p><span class="fw-bolder">ID Registro: </span>{{$item->id}}</p></div>
                                <div class="col-12"><p><span class="fw-bolder">Tipo de Movimiento: </span>{{$item->TIPOMOVIMIENTO}}</p></div>
                                <div class="col-12"><p><span class="fw-bolder">Tipo Material: </span>{{$item->TIPO}}</p></div>
                                <div class="col-12"><p><span class="fw-bolder">Costo: </span>${{ number_format($item->COSTO, 2) }}</p></div>
                                <div class="col-12"><p><span class="fw-bolder">MAC3: </span>{{ $item->MAC3 ?? 'N/A' }}</p></div>
                                <div class="col-12"><p><span class="fw-bolder">Naturaleza: </span>{{$item->Naturaleza}}</p></div>
                                <div class="col-12"><p><span class="fw-bolder">Modificado Por: </span>{{$item->Modificado_por ?? 'Sin modificaciones' }}</p></div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Modal de Confirmación de Eliminación -->
            <div class="modal fade" id="confirmModal-{{$item->id}}" tabindex="-1" aria-labelledby="confirmModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="confirmModalLabel">Mensaje de confirmación</h1>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            ¿Seguro que quieres eliminar este movimiento de material del registro?
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                            <!-- Ajusta el nombre de la ruta según tu archivo web.php -->
                            <form action="{{ route('movimientos.destroy', ['movimiento' => $item->id]) }}" method="post">
                                @method('DELETE')
                                @csrf
                                <button type="submit" class="btn btn-danger">Confirmar</button>
                            </form>
                        </div>
                    </div>
                </div>
            </div>

            @endforeach
        </tbody>
    </table>
  </div>
    <div class="d-flex justify-content-center mt-3" id="laravel-pagination">
        {!! $movimientos->links('pagination::bootstrap-5') !!}
    </div>

</div>

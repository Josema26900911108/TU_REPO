...
<div class="container-fluid px-4">
    <h1 class="mt-4 text-center">Cobrar Venta</h1>
...
<!-- Encabezado con botón para colapsar/maximizar -->
<div class="text-white bg-secondary p-2 d-flex justify-content-between align-items-center">
    <div class="text-center w-100 m-0">
        
        <svg xmlns="http://w3.org" width="16" height="16" fill="currentColor" class="bi bi-book-half" viewBox="0 0 16 16">
    ...
    <button type="button" id="btn-toggle-tabla" class="btn btn-sm btn-light font-weight-bold" style="min-width: 30px;">+</button>
</div>

<!-- Contenedor colapsable (Inicia oculto con d-none) -->
<div id="wrapper-tabla-contable" class="d-none mt-2">
    ...
    <select name="comprobante_id" id="comprobante_id" class="form-control selectpicker" title="Selecciona">
        @foreach ($comprobantes as $item)
            <option value="{{ $item->id }}" 
                @selected($selectedItemIdcomp ? ($item->id == $selectedItemIdcomp) : ($item->defauldoc == 1))>
...
document.getElementById("SKU").addEventListener("keydown", function (e) {
    ...
    document.addEventListener("DOMContentLoaded", function() {
         ...
         const btnToggle = document.getElementById("btn-toggle-tabla");
    const wrapperTabla = document.getElementById("wrapper-tabla-contable");

    btnToggle.addEventListener("click", function () {
        // Intercambia la clase d-none para mostrar u ocultar la tabla
        if (wrapperTabla.classList.contains("d-none")) {
            wrapperTabla.classList.remove("d-none");
            btnToggle.textContent = "−"; // Signo de menos al estar maximizado
        } else {
            wrapperTabla.classList.add("d-none");
            btnToggle.textContent = "+"; // Signo de más al estar oculto
        }
    });

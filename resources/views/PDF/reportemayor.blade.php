...
<div class="container">
    ...
    <h4 class="mb-3">Libro Mayor</h4>

    <form method="GET" action="{{ route('mayor.reporte') }}" class="row g-3 mb-4">
    ...
    {{-- Exportar a Excel --}}
    <form method="GET" action="{{ route('mayor.export.excel') }}" class="d-inline">
        ...
        {{-- Exportar a PDF --}}
    <form method="GET" action="{{ route('mayor.export.pdf') }}" class="d-inline">
        ...
        <tbody>
            @foreach($asientos as $a)
                <tr>
                    ...
                    <td>{{ number_format($a->SaldoLinea, 2) }}</td>
                    <td>{{ $a->usuario }}</td>
@php
    $SaldoFinal += $a->SaldoLinea;
    $totalDebe += $a->Debe;
    $totalHaber += $a->Haber;
@endphp
                </tr>
        ...
        <tfoot></tfoot>
            <tr>
                <th colspan="3" class="text-end">Saldo Final:</th>
                <th>{{ number_format($totalDebe, 2) }}</th>
                <th>{{ number_format($totalHaber, 2) }}</th>
                <th>{{ number_format($SaldoFinal, 2) }}</th>
                <th></th>
            </tr>
    </table>
